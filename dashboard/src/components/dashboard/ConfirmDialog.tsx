...
export function ConfirmDialog({
  ...
  return (
    ...
    >
      <div className="absolute inset-0 bg-[rgba(15,23,42,0.45)]" onClick={loading ? undefined : onCancel} />
      <div className="relative w-full max-w-sm rounded-2xl border border-(--line) bg-(--surface-strong) p-5 shadow-2xl">
        ...
        <div className="mt-5 flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
          <button
            ...
            onClick={onCancel}
            disabled={loading}
            className="rounded-xl border border-(--line) px-4 py-2 text-xs font-semibold text-(--sea-ink) transition hover:border-(--lagoon) disabled:cursor-not-allowed disabled:opacity-50 dark:text-zinc-200"
          >
            {cancelLabel}
          </button>
          <button
            ...
            onClick={onConfirm}
            disabled={loading}
            className={`inline-flex items-center justify-center gap-1.5 rounded-xl px-4 py-2 text-xs font-semibold transition disabled:cursor-not-allowed disabled:opacity-60 ${confirmClass}`}
          >
